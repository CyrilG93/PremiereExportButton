...
function getProjectExportsPath() {
    try {
        ...
        if (isWindows) {
            // Simple replaceAll wasn't working in ExtendScript sometimes, regex is safer
        ...
        // Remove PROJET folder (go up one level) if it exists
        // Check if the last folder is "PROJET"
        if (cleanParts.length > 0 && cleanParts[cleanParts.length - 1] === "PROJET") {
            cleanParts.pop();
        }
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file instanceof File) {
                ...
                if (fileNameLower.indexOf(baseNameLower + "_v") === 0) {
                    ...
                    // Find where the number ends (at the dot or end of string)
                    var dotIndex = afterPrefix.lastIndexOf(".");
                    var versionStr = "";

                    if (dotIndex > 0) {
                        versionStr = afterPrefix.substring(0, dotIndex);
                    } else {
                        versionStr = afterPrefix;
                    }
